...
--   6. A person can have many cars, but every car that person owns can only have one owner.
...
SELECT genres.id, genres.name, AVG(milliseconds) AS average_track_length
FROM genres
JOIN tracks
  ON (tracks.genre_id = genres.id)
GROUP BY genres.id
...
SELECT albums.id, albums.title, AVG(milliseconds) AS average_track_length
FROM albums
JOIN tracks
  ON (tracks.album_id = albums.id)
GROUP BY albums.id
ORDER BY average_track_length
DESC
LIMIT 5;
...
SELECT albums.id, albums.title, SUM(unit_price) AS album_price
FROM albums
JOIN tracks
  ON (tracks.album_id = albums.id)
GROUP BY albums.id
...
SELECT invoice_lines.id, invoice_lines.tracks_id, SUM(unit_price) AS total_track_sales FROM invoice_lines JOIN tracks ON (tracks.id = invoice_lines.track_id) GROUP BY invoice_lines.tracks_id ORDER BY total_track_sales DESC LIMIT 10;
