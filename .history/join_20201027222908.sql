...
SELECT genres.id, genres.name, SUM(milliseconds) AS tracks_length
FROM genres
JOIN tracks
  ON (tracks.genre_id = genres.id)
GROUP BY genres.id
ORDER BY tracks_length DESC
LIMIT 5;
...
SELECT albums.id, albums.name, sum(milliseconds) AS track_length FROM albums JOIN track ON (albums.id = album_id) GROUP BY albums.id ORDER BY track_length LIMIT 5;
