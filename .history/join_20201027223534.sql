...
SELECT albums.id, albums.title, sum(milliseconds) AS track_length FROM albums JOIN tracks ON (tracks.album_id = albums.id) GROUP BY albums.title ORDER BY track_length LIMIT 5;
