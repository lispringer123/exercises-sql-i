...
SELECT albums.id, albums.title, SUM(milliseconds) AS album_length FROM albums JOIN track ON (tracks.album_id = albums.id) GROUP BY albums.id ORDER BY album_length DESC LIMIT 10;
