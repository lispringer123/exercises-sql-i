...
SELECT invoice_lines.id, invoice_lines.track_id, SUM(unit_price) AS total_track_sales FROM invoice_lines JOIN tracks ON (tracks.id = invoice_lines.track_id) GROUP BY invoice_lines.track_id ORDER BY total_track_sales DESC LIMIT 10;
